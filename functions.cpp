...
vector<int> unionPrimeDivsors(int x, int y) {
    ...
    vector<int> unionSet = primeDivisorsX;
    ...
    for (int i = 0; i < primeDivisorsY.size(); ++i) {
        ...
        if (!found) {
            unionSet.push_back(primeDivisorsY[i]);
        }
    }
    return unionSet;
}
...
vector<long long > findFirstNPerfectNumbers(int n) {
    ...
    int  number = 2;
    while (perfectNumbers.size() < number) {
        long long mersenne=mersennePrimeNumbers(number);
        if (isPrime(mersenne)) {
            long long  perfectNumber = perfectNumberFromMersenne(number);
